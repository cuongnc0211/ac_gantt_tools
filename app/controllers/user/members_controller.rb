class User::MembersController < User::BaseController
  ...
  def create
    @user_member = Member.new(user_member_params)

    respond_to do |format|
      if @user_member.save
        format.html { redirect_to user_member_path(@user_member), notice: 'Member was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @user_member.update(user_member_params)
        format.html { redirect_to user_member_path(@user_member), notice: 'Member was successfully updated.' }
  ...
  private
    ...
    def set_user_member
      @user_member = Member.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def user_member_params
      params.require(:member).permit :name, :color
    end
end
